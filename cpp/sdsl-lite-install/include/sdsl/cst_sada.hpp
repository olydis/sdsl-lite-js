...
public:
	...
	{
		{
			...
			if (lcp.size() > 0) {
				...
				while (stack.size() >
					   1) { //remove all elements except the zero from stack for next run
					stack.pop();
	...
	node_type child(node_type v, const char_type c, size_type& char_pos) const
	{
		...
		if (cc == 0 and
			c != 0) // TODO: aendere char2comp so ab, dass man diesen sonderfall nicht braucht
			return root();
		...
		while (true) {
			...
			if (char_pos >=
				char_ex_max_pos) // if the current char is lex. greater than the searched char: exit
				return root();
	...
	size_type inv_id(size_type id)
	{
		if (id < size()) { // the corresponding node is a leaf
			...
			while (rb - lb > 1) {
				...
				size_type mid_id =
				m_bp_support.rank(mid - 1) -
				m_bp_rank10(
				mid); // Note: mid-1 is valid of mid is of type ``size_type'' as us the parameter of rank
				if (mid_id < id) {
					lb = mid;
				} else { // mid_id >= x
					rb = mid;
