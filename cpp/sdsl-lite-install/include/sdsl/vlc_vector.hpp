...
vlc_vector<t_coder, t_dens, t_width>::operator[](const size_type i) const
{
	...
	return (t_coder::template decode<false, false, int*>(
		   m_z.data(), m_sample_pointer[idx], i - t_dens * idx + 1)) -
		   1;
}
...
vlc_vector<t_coder, t_dens, t_width>::vlc_vector(int_vector_buffer<int_width>& v_buf)
{
	...
	m_sample_pointer =
	int_vector<>(samples + 1, 0, bits::hi(z_size + 1) + 1); // add 1 for last entry
